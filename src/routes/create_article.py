...
def create_article_sse_serper(topic: str, query_params: dict[str, str]):
    try:
        from configs import argparser, serper, anthropic
        ...
        yield json.dumps({"event_id": 1, "message": "Have successfully set up llm provider", "current_progress": "10", "is_done": False, "status_code": 200}) + '\n\n'
        ...
        print("Successfully got rm from serper.com!\nRunning runner now!\n")
        yield json.dumps({"event_id": 2, "message": "Have successfully set up search provider, running storm runner now", "current_progress": "20", "is_done": False, "status_code": 200}) + '\n\n'
        ...
        yield json.dumps({"event_id": 3, "message": "Have successfully finished the article generation process", "current_progress": "50", "status_code": 200, "is_done": False}) + '\n\n'
        runner.summary()
    
        directory = runner.article_output_dir
        print(directory)
        yield json.dumps({"event_id": 4, "message": "Have finished summerizing article, beginning process to format article content", "current_progress": "80", "is_done": False, "status_code": 200}) + '\n\n'
        print("Finished running runner!\nParsing file now to send to frontend!\n")
        ...
        try:
            ...
            if(os.environ.get("PDF_STORAGE_SERVICE") == "supabase"):
                yield json.dumps({"event_id": 5, "message": "Updating article in database", "current_progress": "90","status_code": 200, "is_done": False}) + '\n\n'
                try:
                    ...
                    supabase_client.table("storm_generation").insert(
                        ...
                        "url_to_unified_index": OrderedDict(sorted(urls_to_unified_indexes.items())),
                        "id": str(custom_uuid),
                        "pdf_url": "",
                        "created_at": datetime.now().isoformat(),
                    }
                    ).execute()
                    yield json.dumps({"event_id": 6, "message": "Have successfully uploaded to db, sending article content back", "current_progress": "100", "status_code": 200, "is_done": True, "id": str(custom_uuid)}) + '\n\n'
                except Exception as e:
                    yield json.dumps({"event_id": 6, "message": f"Failed to upload article to database: {str(e)}", "current_progress": "0","status_code": 500, "is_done": False}) + '\n\n'
        except Exception as e:
            yield json.dumps({"event_id": 5, "message": f"Failed parsing file, error is: {e}", "current_progress": "0","status_code": 500, "is_done": False}) + '\n\n'
        print("Fully parsed file, sending response back!\n")
    except Exception as e:
        print("Error in create_article_sse_serper: ", e)
        yield json.dumps({"event_id": 1, "message": "Failed to create article", "current_progress": "0", "is_done": True, "status_code": 500}) + '\n\n'
